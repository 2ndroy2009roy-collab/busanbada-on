...
export default function Home() {
  ...
  const [shopInfo, setShopInfo] = useState<{ name: string; kind: string; distance: string; open: string } | null>(null);
  ...
  const facilities = [["🚻","화장실","120m · 도보 2분"],["🚿","샤워장","220m · 운영 중"],["🦶","세족장","180m · 운영 중"],["🅿️","주차장","350m · 28자리"],["🏪","편의점","80m · 24시간"],["🍴","맛집","260m · 도보 4분"],["☕","카페","310m · 도보 5분"],["🛟","안전시설","해변 중앙 · 운영 중"]];
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      <section className={`map-section ${mapExpanded ? "expanded" : ""}`}><div className="section-title"><div><span>🗺️ 주변 편의시설</span><h2>한눈에 보는 시설 지도</h2></div><button className="more" onClick={() => setMapExpanded(!mapExpanded)}>{mapExpanded ? "닫기" : "전체보기"}</button></div><div className="map-card"><div className="map-water">{pick.name}<span>해변</span></div><span className="pin p1">🚻</span><span className="pin p2">🏪</span><span className="pin p3">🅿️</span><span className="pin p4">☕</span>{mapExpanded && <><span className="pin p5">🚿</span><span className="pin p6">🦶</span><span className="pin p7">🛟</span><span className="pin p8">🍴</span></>}<button className="shop-pin shop1" aria-label="바다상점 정보" onClick={() => setShopInfo({name: `${pick.name.replace("해수욕장", "")} 바다상점`, kind: "편의점 · 간식", distance: "현재 위치에서 80m · 도보 1분", open: "영업 중 · 24시간"})}>🏪</button><button className="shop-pin shop2" aria-label="오션뷰 카페 정보" onClick={() => setShopInfo({name: "오션뷰 카페", kind: "카페 · 디저트", distance: "현재 위치에서 310m · 도보 5분", open: "영업 중 · 10:00~21:00"})}>☕</button>{shopInfo && <div className="shop-popup"><button className="popup-close" onClick={() => setShopInfo(null)} aria-label="가게 정보 닫기">×</button><span>🏪</span><div><b>{shopInfo.name}</b><small>{shopInfo.kind}</small><p>📍 {shopInfo.distance}</p><em>● {shopInfo.open}</em></div></div>}<span className="walk">현재 위치에서 7분</span></div><div className="facility-filter">{["전체","🚻 화장실","🚿 샤워장","🅿️ 주차장","🍴 맛집"].map(f => <button className={filter===f ? "active" : ""} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="facility-list">{facilities.filter(x => filter === "전체" || filter.includes(String(x[1]))).slice(0, mapExpanded ? facilities.length : 4).map(x=><div key={String(x[1])}><span>{x[0]}</span><b>{x[1]}</b><small>{x[2]}</small><button>›</button></div>)}</div></section>
