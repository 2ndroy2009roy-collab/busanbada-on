...
const quick = ["🌊 물놀이", "🏄 서핑", "🌅 노을", "📸 사진", "💑 데이트", "👨‍👩‍👧 가족", "😌 한적한 곳", "🔥 핫플"];
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      <section className="map-section"><div className="section-title"><div><span>🗺️ 주변 편의시설</span><h2>한눈에 보는 시설 지도</h2></div><button className="more">전체보기</button></div><div className="map-card"><div className="map-water">{pick.name}<span>해변</span></div><span className="pin p1">🚻</span><span className="pin p2">🏪</span><span className="pin p3">🅿️</span><span className="pin p4">☕</span><span className="walk">현재 위치에서 7분</span></div><div className="facility-filter">{["전체","🚻 화장실","🚿 샤워장","🅿️ 주차장","🍴 맛집"].map(f => <button className={filter===f ? "active" : ""} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="facility-list">{facilities.filter(x => filter === "전체" || filter.includes(String(x[1]))).slice(0,4).map(x=><div key={String(x[1])}><span>{x[0]}</span><b>{x[1]}</b><small>{x[2]}</small><button>›</button></div>)}</div></section>
      ...
      <section className="compare"><div className="section-title"><div><span>⚖️ 바다 비교</span><h2>어디로 갈지 고민된다면</h2></div></div><div className="compare-table"><div><b>구분</b><b>{ranked[0].name.replace("해수욕장","")}</b><b>{ranked[1].name.replace("해수욕장","")}</b><b>{ranked[2].name.replace("해수욕장","")}</b></div><div><span>혼잡도</span><span>{ranked[0].crowd}</span><span>{ranked[1].crowd}</span><span>{ranked[2].crowd}</span></div><div><span>물놀이</span><span>좋음</span><span>좋음</span><span>좋음</span></div></div></section>
    </section>
    <nav><button className="active">🏠<span>홈</span></button><button>🌊<span>바다</span></button><button>🗺️<span>지도</span></button><button>✨<span>AI코스</span></button><button>👤<span>MY</span></button></nav>
  </main>;
}
